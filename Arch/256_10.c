...
void compute(int index) {
  for(long i=0; i<MAXFLOPS_ITERS; i++) {
      ...
      farr[index][16] = _mm256_fmadd_ps(mult, farr[index][16], farr[index][17]);
      farr[index][18] = _mm256_fmadd_ps(mult, farr[index][18], farr[index][19]);
    }
}
...
float summarize() {
  for(int t=0; t<N_THREADS; t++) {
    ...
    farr[t][16] = _mm256_add_ps(farr[t][16], farr[t][17]);
    farr[t][18] = _mm256_add_ps(farr[t][18], farr[t][19]);
    ...
    farr[t][16] = _mm256_add_ps(farr[t][16], farr[t][18]);
    ...
    farr[t][0]  = _mm256_add_ps(farr[t][0], farr[t][16]);
  }
