...
TYPE farr_nehalem[MAX_NUMBER_THREADS][SIZE] __attribute__((aligned(64)));

void compute_nehalem(TYPE *farr, TYPE mult, int index) {
  farr = farr_nehalem[index];
  ...
  for(long i=0; i < BENCHMARK_CPU_ITERS; i++) {
    ...
    farr[12] = _mm_mul_ss(farr[12], farr[13]);
    farr[14] = _mm_mul_ss(farr[14], farr[15]);
  }
}
